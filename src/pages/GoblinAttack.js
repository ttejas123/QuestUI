...
function App() {
  ...
  return (
    <div className="appBG flex justify-center items-center flex-col py-5 h-screen relative">
      ...
      {/* items */}
      <div className={`absolute z-10 w-12 hover:opacity-80 cursor-pointer font-extrabold text-2xl md:text-2xl lg:text-3xl xl:text-4xl text-white z-50 top-[5.5%] right-[50%] outline-title ${visible1 ? "" : "hidden"}`}>Items</div>
      <img onClick={() => setvisible(!visible1)} src={cross} className={`absolute z-10 w-12 hover:opacity-80 cursor-pointer z-50 top-[6.5%] right-[5%] md:right-[5%] lg:right-[11%] xl:right-[15%] ${visible1 ? "" : "hidden"}`}/>
      <div className={`w-9/12 h-4/5 md:w-10/12 md:h-4/5 lg:w-9/12 lg:h-4/5 xl:w-8/12 xl:h-4/5 bg-[#000000] rounded-2xl flex justify-center absolute top-[10%] z-40 ${visible1 ? "" : "hidden"}`}>
